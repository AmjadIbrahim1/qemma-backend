...
class AuthService {
  ...
  async registerLocal(data) {
    ...
    await this.createRoleRecord(user.id, role, division, subject, teacherName, studentUsername);
  ...
  async loginClerk(
    ...
    try {
      ...
      if (user) {
        ...
        await this.ensureRoleRecord(user.id, user.role, division, subject, teacherName, studentUsername);
        ...
        user = await prisma.$transaction(async (tx) => {
          ...
          const parentId = await this._createRoleRecordInTransaction(
            tx, newUser.id, role, division, subject, teacherName,
          );
  ...
  // ─────────────────────────────────────────────────────────────
  async createRoleRecord(userId, role, division = null, subject = null, teacherName = null, studentUsername = null) {
  ...
  // ─────────────────────────────────────────────────────────────
  async _createRoleRecordInTransaction(tx, userId, role, division = null, subject = null, teacherName = null) {
    switch (role) {
      case 'student':
  ...
  // ─────────────────────────────────────────────────────────────
  async ensureRoleRecord(userId, role, division = null, subject = null, teacherName = null, studentUsername = null) {
  ...
  async _linkParentToStudent(parentId, studentUsername) {
    try {
      ...
      if (studentRecord.parentId === parentId) {
