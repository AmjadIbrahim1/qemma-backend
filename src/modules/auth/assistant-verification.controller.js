  ...
  class AssistantVerificationController {
      ...
      try {
        ...
        res.status(200).json({
          ...
          data: {
            ...
            code: result.code, // ⚠️ remove in production
          },
        });
      } catch (error) {
        next(error);
    ...
    async getTeacher(req, res, next) {
      try {
        ...
        const teacher =
          await assistantVerificationService.getTeacherByUsername(username);
    ...
    async sendParentCode(req, res, next) {
      try {
        ...
        const result =
          await assistantVerificationService.sendParentVerificationCode(
    ...
    async getStudent(req, res, next) {
      try {
        const { username } = req.params;
        const student =
          await assistantVerificationService.getStudentByUsername(username);
