...
// CHANGE: thumbnail stored as base64 string (from req.body.thumbnailBase64)
//         instead of saving to the local file system via multer.
...
class CoursesService {
  ...
  async createCourse(userId, data) {
    ...
    const {
      ...
      // ── FIX: base64 instead of multer file ──
      thumbnailBase64,
    } = data;
    ...
    if (!category?.trim())    throw Object.assign(new Error('التصنيف مطلوب'),             { statusCode: 400 });
  ...
  async getPublishedCourses({ subject, level, search, page = 1, limit = 50 } = {}) {
  ...
  async updateCourse(courseId, userId, data) {
    ...
    // Determine new thumbnail: if base64 provided use it, else keep existing
  ...
  _formatTeacherProfile(teacher) {
    ...
    return {
      ...
      booksCount:   teacher._count.books,
